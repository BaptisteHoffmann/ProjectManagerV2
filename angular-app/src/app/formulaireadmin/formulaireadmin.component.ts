...
} from '../services/demandeclient.service';
import {
  ResearchBDDService
...
export class FormulaireadminComponent implements OnInit {
  ...
  constructor(
      ...
      private DemandeAdminService: DemandeClientService,
  ...
  ngOnInit() {
      ...
      this.DemandeAdminService.getFormulaireAdmin(this.id).subscribe((data: string[]) => {
     ...
     for (this.index = 0; this.index < this.lignechiffrageprojets.length; this.index++) {
      ...
      this.calcul = this.calcul + this.sousetapeForm.get('total').value;
          ...
          if (this.lignechiffrageprojets && this.lignechiffrageprojets[0] && this.lignechiffrageprojets[0].fk_etat_id !== (2 || undefined)) {
  ...
  formulaire() {
    ...
    this.sousetapeForm.get('total').setValue((this.sousetapeForm.get('tjm').value * this.sousetapeForm.get('charge').value));
      ...
      this.id = this.route.snapshot.paramMap.get('id');
      this.DemandeAdminService.setFormulaireAdmin(this.id, this.sousetapeForm).subscribe((data2: string[]) => {
          //Déclenche le clic pour les fênetres pop up
          document.getElementById('openModal').click();
          data2.forEach(element => this.lignechiffrageprojets.push(element));
  ...
  deleteEtape(id) {
      ...
      this.DemandeAdminService.setDeleteFormulaire(id, this.iddemande).subscribe((data: string[]) => {
          this.lignechiffrageprojets = data;
  ...
  onClickValiderChiffrage() {
      this.id = this.route.snapshot.paramMap.get('id');
      this.etatProjet.setEtatProjetTraiteAdmin(this.id, 3);
      this.etatProjet.setValeurTotal(this.id, this.calcul);
